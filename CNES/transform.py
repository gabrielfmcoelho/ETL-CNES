def unzip_specific_files(path_temp, path_unziped, target_tables, date):
    ...
    os.makedirs(path_unziped+ '/' + 'BASE_DE_', exist_ok=True)
    existent_files = os.listdir(path_unziped + '/' + 'BASE_DE_')
    for table in target_tables:
        target = str(table)+str(date)+'.csv'
        if target in existent_files:
            continue
        with zipfile.ZipFile(path_temp + '/' + filename, 'r') as zip_ref:
                    zip_ref.extract(target, path=path_unziped + '/' + 'BASE_DE_')
    return
...
def compile_info(path_final, start_date, end_date):
    ...
    for count, file in enumerate(os.listdir(path_final)):
        df = pd.read_csv(path_final+'/'+file)
        df_merged = df if count == 0 else pd.concat([df_merged, df])
        os.unlink(path_final+'/'+file)

    filename_final = file[0:4]+str(start_date)+'-'+str(end_date)
    try:
        df_merged.to_csv(path_final + '/' + filename_final + '.csv', sep=';', index=False, encoding='utf-8')
    except:
        os.unlink(path_final + '/' + filename_final + '.csv')
        df_merged.to_csv(path_final + '/' + filename_final + '.csv', sep=';', index=False, encoding='utf-8')
...
def move_and_clean(path_unziped, path_final, path_datasets):
    ...
    try:
        shutil.move(path_final+'/'+file, path_datasets)
    except:
        os.unlink(path_datasets+'/'+file)
        shutil.move(path_final+'/'+file, path_datasets)

    for file in os.listdir(path_unziped):
